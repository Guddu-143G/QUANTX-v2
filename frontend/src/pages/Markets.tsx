...
export default function Markets() {
  ...
  useEffect(() => {
    const assetTokens = assets.data?.map(a => a.token).filter(Boolean) as number[] || [];
    // We added token optionally to indices in market.ts, but TS might complain if it's not typed. Let's just cast.
    const indexTokens = (indices.data as any[])?.map(i => i.token).filter(Boolean) as number[] || [];
    const tokens = [...assetTokens, ...indexTokens];
    if (tokens.length > 0) subscribe(tokens);
  }, [assets.data, indices.data]);

  const rows = useMemo(() => {
    const a = assets.data ?? [];
    
    // Merge live ticks
    const liveAssets = a.map(asset => {
      if (asset.token && ticks[asset.token]) {
        const t = ticks[asset.token];
        const lastPrice = t.last_price;
        const change = t.change ?? (t.ohlc && t.ohlc.close ? ((lastPrice - t.ohlc.close) / t.ohlc.close * 100) : asset.chgPct);
    ...
    if (view === "Gainers") return [...liveAssets].sort((x, y) => y.chgPct - x.chgPct).slice(0, 10);
    if (view === "Losers") return [...liveAssets].sort((x, y) => x.chgPct - y.chgPct).slice(0, 10);
    if (view === "Most active") return [...liveAssets].sort((x, y) => y.volumeCr - x.volumeCr).slice(0, 10);
    return liveAssets;
  }, [assets.data, view, ticks]);

  const liveIndices = useMemo(() => {
    const idx = indices.data ?? [];
    return idx.map(i => {
      // @ts-ignore
      if (i.token && ticks[i.token]) {
        // @ts-ignore
        const t = ticks[i.token];
        const lastPrice = t.last_price;
        const change = t.change ?? (t.ohlc && t.ohlc.close ? ((lastPrice - t.ohlc.close) / t.ohlc.close * 100) : i.chgPct);
        return {
          ...i,
          value: lastPrice,
          chgPct: change
  ...
  return (
    <>
      ...
      <Panel level={3} title={view === "All" ? "Instrument Screener" : view === "Most active" ? "Most Active" : `Top ${view}`} sub={`${rows.length} instruments · click any row for the full research view`} bodyClass="p-0">
