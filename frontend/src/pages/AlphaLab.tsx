...
export default function AlphaLab() {
  ...
  const runBacktest = async () => {
    setRunning(true);
    try {
      // Map frontend keys to backend expected keys if necessary, or just send directly
      // Backend expects lowercase keys to match factor implementations
      ...
      await alphaService.runBacktest(backendWeights);
      
      setDirty(false);
      push({
        title: "Alpha backtest complete",
        body: "Composite re-estimated on the PiT NIFTY 200 universe.",
        metrics: [{ k: "Sharpe", v: "1.82", tone: "pos" }, { k: "CAGR", v: "21.4%", tone: "pos" }, { k: "MaxDD", v: "−8.4%", tone: "neg" }],
      });
    } catch (err: any) {
      push({
        title: "Validation Failed",
        body: err.message || "Failed to start backtest. Check composite weights.",
        tone: "neg"
      });
    } finally {
      setRunning(false);
          ...
          <Panel level={3} title="Composite Configuration" sub="Signal blend weights"
            ...
            {total !== 100 && (
              <div className="mt-3 rounded-[6px] border border-warn/25 bg-warn/6 px-2.5 py-2 text-[10.5px] leading-relaxed text-warn">
                ▲ Weights sum to {total}%. Normalise to 100% before promoting the composite. Backend will reject this configuration.
              </div>
            )}
