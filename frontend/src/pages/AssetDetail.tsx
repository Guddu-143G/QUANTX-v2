...
export default function AssetDetail({ ticker }: { ticker: string }) {
  ...
  const news = useAsync(() => newsService.feed(ticker), [ticker]);
  const analytics = useAsync(() => marketService.analytics(ticker), [ticker]);
  ...
  const liveAsset = useMemo(() => {
    if (!a) return a;
    if (a.token && ticks[a.token]) {
      const t = ticks[a.token];
      const lastPrice = t.last_price;
      const change = t.change ?? (t.ohlc && t.ohlc.close ? ((lastPrice - t.ohlc.close) / t.ohlc.close * 100) : a.chgPct);
      return { ...a, price: lastPrice, chgPct: change };
    }
    return a;
  }, [a, ticks]);
  ...
  const liveCandles = useMemo(() => {
    ...
    if (a?.token && ticks[a.token]) {
      ...
      const tString = `${String(now.getHours()).padStart(2, '0')}:${String(bucketMinutes).padStart(2, '0')}`;
      
      const lastCandle = cData[cData.length - 1];
      
      if (lastCandle && lastCandle.t === tString) {
        lastCandle.c = lastPrice;
        if (lastPrice > lastCandle.h) lastCandle.h = lastPrice;
        if (lastPrice < lastCandle.l) lastCandle.l = lastPrice;
        if (t.volume_traded) lastCandle.v = t.volume_traded;
      } else if (lastCandle) {
        cData.push({
          t: tString,
          o: lastPrice,
    ...
    <>
          ...
          <div className="flex flex-wrap items-end justify-between gap-4">
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2.5">
                <h1 className="text-[20px] font-semibold uppercase tracking-[-0.01em] text-txt-primary sm:text-[24px]">{liveAsset.name}</h1>
                <span className="mono rounded-[4px] border border-line-subtle px-1.5 py-0.5 text-[10px] text-txt-muted">{liveAsset.ticker}</span>
                <Badge tone="neu">{liveAsset.exch}</Badge>
                <Badge tone="neu">{liveAsset.sector}</Badge>
                {holding && <Badge tone="info">Held · {holding.weight.toFixed(1)}%</Badge>}
              </div>
              <div className="mt-2.5 flex flex-wrap items-baseline gap-3">
                <span className="tnum text-[28px] font-semibold leading-none tracking-[-0.025em] text-txt-primary">₹{num(liveAsset.price, 2)}</span>
                <span className={cn("mono text-[14px]", liveAsset.chgPct >= 0 ? "text-pos" : "text-neg")}>
                  {liveAsset.chgPct >= 0 ? "↑ +" : "↓ −"}{Math.abs(liveAsset.chgPct).toFixed(2)}%
                  <span className="ml-1.5 text-[12px] text-txt-muted">({liveAsset.chgPct >= 0 ? "+" : "−"}₹{Math.abs((liveAsset.price * liveAsset.chgPct) / 100).toFixed(2)})</span>
                </span>
                <span className="label-xs text-txt-disabled">Last 15:30:04 IST</span>
              </div>
            </div>
            <div className="flex items-center gap-1.5">
              <Sparkline data={liveAsset.spark} width={140} height={38} strokeWidth={1.1} />
              <Button
                size="sm"
                variant={isWatched(ticker.toUpperCase()) ? "primary" : "ghost"}
                icon={isWatched(ticker.toUpperCase()) ? BookmarkCheck : Bookmark}
                onClick={() => toggleWatchlist(ticker.toUpperCase())}
              >
                {isWatched(ticker.toUpperCase()) ? "Watching" : "Watch"}
              </Button>
              <Button size="sm" variant="primary" icon={Plus} onClick={() => setTradeOpen(true)}>Add to portfolio</Button>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="mb-4 grid grid-cols-3 gap-x-3 gap-y-3 rounded-[8px] border border-line-subtle bg-surface/50 px-3.5 py-3 sm:grid-cols-4 lg:grid-cols-8">
        {asset.loading || !a ? Array.from({ length: 8 }).map((_, i) => <Skeleton key={i} className="h-9" />) : (
          <>
            <StatCell k="Mkt Cap" v={`₹${(liveAsset.mcapCr / 100000).toFixed(2)} L Cr`} />
            <StatCell k="Turnover" v={`₹${num(liveAsset.volumeCr, 0)} Cr`} />
            <StatCell k="P/E" v={liveAsset.pe.toFixed(1)} />
            <StatCell k="P/B" v={liveAsset.pb.toFixed(1)} />
            <StatCell k="Div yield" v={`${liveAsset.divYield.toFixed(2)}%`} />
            <StatCell k="ROE" v={`${liveAsset.roe.toFixed(1)}%`} />
            <StatCell k="Beta" v={liveAsset.beta.toFixed(2)} />
            <StatCell k="σ 30D" v={`${liveAsset.vol30.toFixed(1)}%`} />
          </>
        )}
      </div>
      ...
      {tab === "overview" && (
        <div className="grid grid-cols-1 gap-3 xl:grid-cols-12">
          <Panel level={3} className="xl:col-span-8" title="Price Chart" sub="Intraday 5-minute candles · NSE consolidated"
            ...
            {candles.loading || !liveCandles ? <ChartSkeleton height={260} /> : (
              <>
                <Candles data={liveCandles} height={260} />
                <div className="mt-3 grid grid-cols-3 gap-2 border-t border-line-subtle pt-3 sm:grid-cols-6">
                  <StatCell k="Open" v={`₹${num(liveCandles[0].o, 2)}`} />
                  <StatCell k="High" v={`₹${num(Math.max(...liveCandles.map((c) => c.h)), 2)}`} tone="pos" />
                  <StatCell k="Low" v={`₹${num(Math.min(...liveCandles.map((c) => c.l)), 2)}`} tone="neg" />
                  <StatCell k="Close" v={`₹${num(liveCandles[liveCandles.length - 1].c, 2)}`} />
                  <StatCell k="VWAP" v={`₹${num(liveCandles.reduce((s, c) => s + c.c, 0) / liveCandles.length, 2)}`} />
                  <StatCell k="Volume" v={`${num(liveCandles.reduce((s, c) => s + c.v, 0) / 100, 1)} L`} />
                </div>
              </>
            )}
          </Panel>
              ...
              {!alpha ? <p className="text-[11.5px] text-txt-muted">No live signal for this instrument.</p> : (
                <>
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="label-xs text-txt-disabled">Composite alpha / QUANTX Score</div>
                      <div className="tnum mt-1 text-[24px] font-semibold leading-none text-txt-primary">
                        {analytics.data ? analytics.data.scores.quantx_score.toFixed(2) : alpha.alpha.toFixed(2)}
                      </div>
                    </div>
                    <RiskBadge level={analytics.data?.scores?.quantx_score > 0.65 ? "BUY" : (analytics.data?.scores?.quantx_score < 0.4 ? "SELL" : "HOLD")} />
                  </div>
                  <div className="mt-3 space-y-2.5">
                    <Row k="ML probability"><ProbCell v={analytics.data ? analytics.data.scores.momentum : alpha.mlProb} /></Row>
                    <Row k="Model confidence">
                      <span className="mono text-[11.5px] text-txt-primary">
                        {analytics.data ? (analytics.data.scores.quantx_score * 100).toFixed(0) : alpha.confidence}%
                      </span>
                    </Row>
        ...
        <div className="grid grid-cols-1 gap-3 lg:grid-cols-12">
          <Panel level={3} className="lg:col-span-6" title="Factor Exposures" sub="Z-scores vs NIFTY 200 cross-section">
            {!alpha ? <p className="text-[11.5px] text-txt-muted">No factor scores available.</p> : (
              <ul className="space-y-3">
                {[
                  { k: "Momentum", v: analytics.data?.scores?.momentum ?? alpha.momentum },
                  { k: "Confidence", v: analytics.data ? `${(analytics.data.scores.quantx_score * 100).toFixed(0)}%` : `${alpha.confidence}%` },
                  { k: "Conviction", v: analytics.data?.scores?.quantx_score > 0.7 ? "HIGH" : "MED" },
                  { k: "Horizon", v: alpha.horizon },
                  { k: "Low volatility", v: 100 - (a?.vol30 ?? 20) * 2 },
                  { k: "RSI (14D)", v: analytics.data?.indicators?.rsi ?? 50 },
                ].map((r) => (
