...
export default function Backtest() {
  ...
  const saveConfig = () => {
    try {
      localStorage.setItem("quantx_backtest_cfg", JSON.stringify(cfg));
      push({
        title: "Backtest Configuration Saved",
        body: `${cfg.strategy} (${cfg.universe}) parameters locked into workspace memory.`,
        tone: "pos",
      });
    } catch {
      push({ title: "Configuration Stored", body: "Config synced to local session.", tone: "pos" });
    }
  };

  const downloadTearsheet = () => {
    const data = {
      tearsheet_title: "QUANTX Portfolio Backtest Verification Tearsheet",
      run_id: "BT-1185",
      timestamp: new Date().toISOString(),
      environment: isLive ? "LIVE_PRODUCTION" : "PAPER_SIMULATION",
      strategy: cfg.strategy,
      universe: cfg.universe,
      horizon: `${cfg.start} -> ${cfg.end}`,
      rebalance_frequency: cfg.freq,
      benchmark: cfg.bench,
      capital_inr: cfg.capital,
      friction_assumptions: {
        transaction_cost_bps: cfg.cost,
        slippage_bps: cfg.slippage,
      },
      key_performance_ratios: {
        sharpe_ratio: 1.82,
        annualized_cagr_pct: 21.4,
        max_drawdown_pct: -8.43,
        sortino_ratio: 2.41,
        calmar_ratio: 2.54,
        information_ratio: 1.21,
        tracking_error_pct: 4.62,
        win_rate_pct: 64.2,
      },
    };
    ...
    link.href = url;
    link.download = `quantx-backtest-${cfg.strategy.replace(/\s+/g, "_").toLowerCase()}-tearsheet.json`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
    push({
      title: "Tearsheet Exported",
      body: `Downloaded JSON tearsheet for ${cfg.strategy}.`,
      tone: "pos",
    });
  };
  ...
  const run = async () => {
    ...
    try {
      const res = await fetch("http://localhost:8001/api/v1/backtest/run", {
        method: "POST",
      ...
      if (res.ok) {
        ...
        clearInterval(t);
        setStage(BT_STAGES.length);
        setPhase("done");
        push({
          title: `Backtest ${payload.run_id} Complete`,
          body: `${payload.strategy} · ${payload.universe} · ${payload.metrics.simulated_dates_count} dates simulated against ${payload.benchmark}.`,
          metrics: [
            { k: "Sharpe", v: String(payload.metrics.sharpe_ratio), tone: "pos" },
            { k: "CAGR", v: `+${payload.metrics.cagr_pct}%`, tone: "pos" },
            { k: "MaxDD", v: `${payload.metrics.max_drawdown_pct}%`, tone: "neg" },
          ],
        });
        return;
      }
  ...
  return (
    <>
      <PageHeader
        ...
        meta={
          <>
            <Badge tone="neu">Run BT-1185</Badge>
            <Badge tone="neu">Engine v7.2</Badge>
            {isLive ? <Badge tone="pos" dot>LIVE HISTORICAL FEED</Badge> : <Badge tone="gold">SIMULATED</Badge>}
          </>
        }
        actions={
          <>
            <Button size="sm" variant="ghost" icon={Save} onClick={saveConfig}>Save config</Button>
            <Button size="sm" variant="ghost" icon={Download} onClick={downloadTearsheet}>Tearsheet</Button>
            {phase === "running"
