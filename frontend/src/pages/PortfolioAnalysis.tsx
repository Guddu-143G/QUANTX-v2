...
export default function PortfolioAnalysis() {
  ...
  async function analyze() {
    ...
    try {
      ...
      const response = await fetch("/api/v1/portfolio/analyze", { method: "POST", body, credentials: "include" });
      const payload = await response.json();
      if (!response.ok) throw new Error(payload.detail || "Analysis request failed.");
      setReport(payload);
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Unable to analyze these files."); }
...
function ReportView({ report }: { report: Report }) {
  ...
  const topPositions = [...report.positions].sort((a, b) => b.weight - a.weight).slice(0, 5);
  return <div className="space-y-4"><div className="grid grid-cols-2 gap-2.5 md:grid-cols-3 xl:grid-cols-6"><Kpi label="Research grade" value={`${report.summary.research_grade} · ${report.summary.research_score}/100`} accent /><Kpi label="Market value" value={inrCompact(report.summary.market_value)} /><Kpi label="Unrealised P&L" value={inrCompact(report.summary.unrealized_pnl)} positive={report.summary.unrealized_pnl >= 0}/><Kpi label="Annualized return" value={percent(p.annualized_return)} /><Kpi label="Volatility" value={percent(p.annualized_volatility)} /><Kpi label="Max drawdown" value={percent(p.max_drawdown)} /></div>
    <div className="grid gap-4 xl:grid-cols-12"><Panel level={3} className="xl:col-span-7" title="Risk & active-return profile" sub="Historical estimates, annualised from shared portfolio dates"><div className="grid gap-x-6 md:grid-cols-2"><MetricRows rows={[["Sharpe ratio", metric(p.sharpe_ratio)], ["Sortino ratio", metric(p.sortino_ratio)], ["Calmar ratio", metric(p.calmar_ratio)], ["Omega ratio", metric(p.omega_ratio)], ["Historical VaR · 95% / 1D", `${percent(r.historical_var_95_1d)} · ${inrCompact(Number(r.var_95_currency))}`], ["Expected shortfall · CVaR", `${percent(r.historical_cvar_95_1d)} · ${inrCompact(Number(r.cvar_95_currency))}`], ["Beta", metric(r.beta)]]}/><MetricRows rows={[["Active return", percent(r.active_return)], ["Tracking error", percent(r.tracking_error)], ["Information ratio", metric(r.information_ratio)], ["Benchmark correlation", metric(r.benchmark_correlation)], ["Observations", String(p.observations ?? "—")], ["Analysis window", `${report.data_quality.first_date} → ${report.data_quality.last_date}`]]}/></div></Panel><Panel level={3} className="xl:col-span-5" title="Concentration monitor" sub={`HHI ${report.concentration.hhi.toFixed(3)} · ${report.concentration.effective_number_of_positions.toFixed(1)} effective names`}><div className="space-y-3">{topPositions.map(pos => <div key={pos.ticker}><div className="flex justify-between text-[11px]"><span className="font-medium text-txt-primary">{pos.ticker}</span><span className="mono text-txt-secondary">{percent(pos.weight)}</span></div><div className="mt-1 h-1.5 overflow-hidden rounded-full bg-surface-active"><div className="h-full rounded-full bg-gradient-to-r from-acc to-acc2" style={{ width: `${Math.min(100, pos.weight * 250)}%` }}/></div></div>)}</div></Panel></div>
    {report.data_quality.warnings.length > 0 && <Panel level={3} title="Data-quality notes" tone="warn"><ul className="space-y-2 text-[11.5px] text-txt-secondary">{report.data_quality.warnings.map(w => <li key={w}>• {w}</li>)}</ul></Panel>}
    <div className="grid gap-4 xl:grid-cols-12"><Panel level={3} className="xl:col-span-7" title="Research findings" sub="Threshold-based observations ranked by materiality"><div className="space-y-2">{report.findings.map(f => <div key={f.title} className="rounded-[7px] border border-line-subtle bg-surface-2 p-3"><div className="flex gap-2"><Badge tone={f.severity === "high" ? "neg" : f.severity === "medium" ? "warn" : "neu"}>{f.severity.toUpperCase()}</Badge><span className="text-[11.5px] font-medium text-txt-primary">{f.title}</span></div><p className="mt-1 text-[11px] leading-relaxed text-txt-secondary">{f.detail}</p></div>)}</div></Panel><Panel level={3} className="xl:col-span-5" title="Data lineage" sub="Valuation and risk metrics are deliberately separated."><MetricRows rows={[["Input rows", report.data_quality.source_rows ? `${report.data_quality.source_rows.holdings} holdings · ${report.data_quality.source_rows.prices} prices` : "—"], ["Common sessions", String(report.data_quality.portfolio_dates)], ["Latest closes", report.data_quality.latest_price_dates?.join(", ") ?? report.data_quality.last_date], ["Data mode", "User-uploaded CSV"], ["Method", "Historical returns + constraints"]]}/></Panel></div>
    <Panel level={3} title="Position diagnostics & allocation proposal" sub="A constrained risk-adjusted scoring proposal — not an executable trade instruction."><div className="overflow-auto"><table className="w-full min-w-[800px] text-left text-[11px]"><thead className="border-b border-line text-txt-muted"><tr><th className="p-2">Ticker</th><th className="p-2">Sector</th><th className="p-2 text-right">Current</th><th className="p-2 text-right">Proposed</th><th className="p-2 text-right">Change</th><th className="p-2 text-right">P&L</th><th className="p-2 text-right">Vol.</th><th className="p-2 text-right">Risk contribution</th><th className="p-2">As of</th></tr></thead><tbody>{report.proposed_allocation.weights.map(w => { const pos = report.positions.find(x => x.ticker === w.ticker)!; return <tr key={w.ticker} className="border-b border-line-subtle text-txt-secondary transition hover:bg-surface-hover/50"><td className="p-2 font-medium text-txt-primary">{w.ticker}</td><td className="p-2">{pos.sector}</td><td className="p-2 text-right mono">{percent(w.current_weight)}</td><td className="p-2 text-right mono text-acc">{percent(w.proposed_weight)}</td><td className="p-2 text-right mono">{percent(w.change)}</td><td className="p-2 text-right mono">{inrCompact(pos.unrealized_pnl)}</td><td className="p-2 text-right mono">{percent(pos.annualized_volatility)}</td><td className="p-2 text-right mono">{metric(pos.marginal_risk_contribution, 3)}</td><td className="p-2 mono">{pos.as_of}</td></tr>})}</tbody></table></div></Panel><p className="px-1 text-[10px] leading-relaxed text-txt-disabled">{report.disclaimer}</p></div>;
}
function Kpi({ label, value, accent, positive }: { label: string; value: string; accent?: boolean; positive?: boolean }) { return <div className={`rounded-[7px] border p-3 ${accent ? "border-acc/30 bg-acc/7" : "border-line bg-surface-1"}`}><div className="label-xs text-txt-muted">{label}</div><div className={`mt-1 mono text-[15px] ${accent ? "text-acc" : positive ? "text-pos" : "text-txt-primary"}`}>{value}</div></div>; }
function MetricRows({ rows }: { rows: string[][] }) { return <ul className="space-y-2.5">{rows.map(([k, v]) => <li key={k} className="flex justify-between gap-3 border-b border-line-subtle pb-2 last:border-0"><span className="text-[11.5px] text-txt-secondary">{k}</span><span className="mono max-w-[62%] text-right text-[11.5px] text-txt-primary">{v}</span></li>)}</ul>; }
