...
export function AlertBanner({ severity, title, children, className, onDismiss }: {
  severity: "INFO" | "SUCCESS" | "WARNING" | "CRITICAL"; title: string; children?: ReactNode; className?: string; onDismiss?: () => void;
}) {
  const cfg = {
    INFO: { i: Info, c: "border-acc2/30 bg-acc2/6", t: "text-acc2" },
    SUCCESS: { i: Check, c: "border-pos/30 bg-pos/6", t: "text-pos" },
  ...
  const I = cfg.i;
  return (
    <div className={cn("flex items-start gap-2.5 rounded-[7px] border px-3 py-2.5", cfg.c, className)}>
