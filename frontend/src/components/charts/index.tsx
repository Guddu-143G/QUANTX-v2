...
export function Sparkline({
  ...
  const { d, area, color } = useMemo(() => {
    if (!data || data.length < 2) return { d: "", area: "", color: C.neu };
...
export function FactorRadarChart({ data, height = 240 }: { data: FactorTilt[]; height?: number }) {
  return (
    <ResponsiveContainer width="100%" height={height}>
      <RadarChart data={data} margin={{ top: 10, right: 20, bottom: 10, left: 20 }}>
        <PolarGrid stroke="#1c2834" />
        <PolarAngleAxis dataKey="factor" tick={{ fill: "#8290a0", fontSize: 10, fontWeight: 500 }} />
        <PolarRadiusAxis angle={30} domain={[0, 100]} tick={{ fill: "#4b5563", fontSize: 8 }} axisLine={false} />
        <Tooltip
          content={({ active, payload }) => {
            if (!active || !payload?.length) return null;
            const p = payload[0].payload as FactorTilt;
            return (
              <TipShell
                label={p.factor}
                rows={[
                  { k: "Portfolio Exposure", v: `${p.portfolio}%`, c: C.acc },
                  { k: "Benchmark Tilt", v: `${p.benchmark}%`, c: C.neu },
                ]}
              />
            );
          }}
        />
        <Radar name="Benchmark" dataKey="benchmark" stroke={C.neu} fill={C.neu} fillOpacity={0.15} strokeDasharray="3 3" strokeWidth={1} />
        <Radar name="Portfolio" dataKey="portfolio" stroke={C.acc} fill={C.acc} fillOpacity={0.32} strokeWidth={1.5} />
      </RadarChart>
    </ResponsiveContainer>
  );
}

/* ───────────────────── Liquidity & Drawdown Flight Gauges ───────────────────── */

export function FlightGauges({
  liquidity = 84.5,
  drawdown = 8.6,
  drawdownLimit = 15.0,
  var95 = 1.84,
  ...
  return (
    <div className="grid grid-cols-1 gap-2.5 sm:grid-cols-3">
      {/* T+1 Liquidity */}
      <div className="flex flex-col justify-between rounded-[7px] border border-line-subtle bg-surface-2 p-3">
        <div className="flex items-center justify-between">
          <span className="label-xs text-txt-muted">T+1 Liquidity Cap</span>
          <span className="mono text-[10px] text-acc font-medium">OPTIMAL</span>
        </div>
        <div className="my-2">
          <div className="flex items-baseline justify-between">
            <span className="tnum text-[18px] font-semibold text-txt-primary">{liquidity.toFixed(1)}%</span>
            <span className="mono text-[10px] text-txt-disabled">of NAV</span>
          </div>
          <div className="mt-1.5 h-1.5 w-full overflow-hidden rounded-full bg-line-subtle">
            <div className="h-full rounded-full bg-acc transition-all duration-700" style={{ width: `${liquidity}%` }} />
          </div>
        </div>
        <span className="text-[9.5px] text-txt-disabled">&lt;10bps market impact</span>
      </div>

      {/* Stress Max Drawdown */}
      <div className="flex flex-col justify-between rounded-[7px] border border-line-subtle bg-surface-2 p-3">
        <div className="flex items-center justify-between">
          <span className="label-xs text-txt-muted">Historical MDD</span>
          <span className="mono text-[10px] text-txt-secondary">{drawdown.toFixed(1)}% / {drawdownLimit.toFixed(1)}%</span>
        </div>
        <div className="my-2">
          <div className="flex items-baseline justify-between">
            <span className="tnum text-[18px] font-semibold text-warn">−{drawdown.toFixed(1)}%</span>
            <span className="mono text-[10px] text-txt-disabled">Cap {drawdownLimit}%</span>
          </div>
          <div className="relative mt-1.5 h-1.5 w-full overflow-hidden rounded-full bg-line-subtle">
            <div className="h-full rounded-full bg-warn transition-all duration-700" style={{ width: `${(drawdown / drawdownLimit) * 100}%` }} />
          </div>
        </div>
        <span className="text-[9.5px] text-txt-disabled">Stress scenario buffer: +{(drawdownLimit - drawdown).toFixed(1)}%</span>
      </div>

      {/* Intraday 95% 1-Day VaR */}
      <div className="flex flex-col justify-between rounded-[7px] border border-line-subtle bg-surface-2 p-3">
        <div className="flex items-center justify-between">
          <span className="label-xs text-txt-muted">Intraday 95% VaR</span>
          <span className="mono text-[10px] text-pos font-medium">PASS</span>
        </div>
        <div className="my-2">
          <div className="flex items-baseline justify-between">
            <span className="tnum text-[18px] font-semibold text-txt-primary">{var95.toFixed(2)}%</span>
            <span className="mono text-[10px] text-txt-disabled">Limit {varLimit.toFixed(2)}%</span>
          </div>
          <div className="relative mt-1.5 h-1.5 w-full overflow-hidden rounded-full bg-line-subtle">
            <div className="h-full rounded-full bg-acc transition-all duration-700" style={{ width: `${(var95 / varLimit) * 100}%` }} />
          </div>
        </div>
        <span className="text-[9.5px] text-txt-disabled">1-Day horizon (Parametric / Hist)</span>
      </div>
    </div>
  );
}
