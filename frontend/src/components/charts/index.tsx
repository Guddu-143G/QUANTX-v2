  ...
  return (
    <ResponsiveContainer width="100%" height={height}>
      <AreaChart data={data} margin={{ top: 4, right: 2, bottom: 0, left: 0 }}>
        ...
        <XAxis dataKey={xKey} tickLine={false} axisLine={false} minTickGap={40} tick={{ fontSize: 9, fill: "#647180" }} />
        <YAxis tickLine={false} axisLine={false} width={45} tick={{ fontSize: 9, fill: "#647180" }} domain={["auto", "auto"]} />
...
export function WaterfallChart({ data, height = 200 }: { data: { name: string; value: number }[]; height?: number }) {
  ...
  return (
    <ResponsiveContainer width="100%" height={height}>
      <BarChart data={waterfallData} margin={{ top: 8, right: 4, bottom: 0, left: -24 }}>
        <CartesianGrid stroke={C.grid} vertical={false} />
        ...
        <Tooltip cursor={{ fill: "rgba(255,255,255,0.025)" }}
          content={({ active, payload, label }) => active && payload?.length
            ? <TipShell label={String(label)} rows={[{ k: "Contribution", v: `${Number(payload[0].payload.value) >= 0 ? "+" : "−"}${Math.abs(Number(payload[0].payload.value)).toFixed(2)}pp` }]} /> : null} />
        <ReferenceLine y={0} stroke="#2e3d4c" />
        <Bar dataKey="range" animationDuration={700} maxBarSize={44}>
          {waterfallData.map((d, i) => <Cell key={i} fill={d.isPositive ? C.acc : C.neg} fillOpacity={0.82} />)}
        </Bar>
      </BarChart>
    </ResponsiveContainer>
  );
