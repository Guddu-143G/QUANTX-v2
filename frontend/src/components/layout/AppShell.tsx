...
import { MobileEmergencyModal } from "../mobile/MobileEmergencyModal";
...
function Sidebar({ collapsed, setCollapsed, onCloseMobile, mobileOpen, isLive, onToggleLive }: {
  collapsed: boolean; setCollapsed: (v: boolean) => void; onCloseMobile: () => void; mobileOpen: boolean;
  isLive: boolean; onToggleLive: () => void;
}) {
      ...
      >
        ...
        <div className="shrink-0 border-t border-line-subtle p-2">
          <div
            ...
            className={cn(
              "mb-2 cursor-pointer rounded-[6px] border transition-all duration-200 select-none",
              isLive
                ? "border-pos/40 bg-pos/10 hover:border-pos/60 hover:bg-pos/15 shadow-[0_0_10px_rgba(46,213,115,0.15)]"
                : "border-warn/25 bg-warn/6 hover:border-warn/40 hover:bg-warn/10",
              collapsed ? "p-1.5 text-center" : "px-2.5 py-2"
            )}
          >
            {collapsed ? (
              <Tooltip content={isLive ? "Environment: LIVE PRODUCTION (Click to toggle)" : "Environment: PAPER TRADING (Click to toggle)"} side="right">
                <span className={cn("mx-auto block h-1.5 w-1.5 rounded-full anim-pulse-dot", isLive ? "bg-pos shadow-[0_0_6px_#2ed573]" : "bg-warn")} />
              </Tooltip>
            ) : (
              <>
                <div className="flex items-center justify-between">
                  <span className="label-xs text-txt-disabled">Environment</span>
                  <span className="mono text-[9px] text-txt-muted hover:text-txt-primary">Toggle</span>
                </div>
                <div className="mt-1 flex items-center gap-1.5">
                  <span className={cn("h-1.5 w-1.5 rounded-full anim-pulse-dot", isLive ? "bg-pos shadow-[0_0_6px_#2ed573]" : "bg-warn")} />
                  <span className={cn("label-xs font-semibold", isLive ? "text-pos" : "text-warn")}>
                    {isLive ? "LIVE PRODUCTION" : "Paper Trading"}
                  </span>
                </div>
              </>
            )}
...
function Topbar({ onMenu, onOpenPalette, onOpenMobileEmergency, crumbs }: {
  onMenu: () => void;
  onOpenPalette: () => void;
  onOpenMobileEmergency: () => void;
  crumbs: string[];
}) {
    ...
    <header className="sticky top-0 z-50 border-b border-line-subtle bg-bg-primary/92 backdrop-blur-xl">
      <div className="flex h-14 items-center gap-3 px-3 sm:px-4">
        ...
        <div className="ml-auto flex shrink-0 items-center gap-1">
          ...
          <Tooltip content="QUANTX Mobile & Emergency Kill-Switch">
            <button
              onClick={onOpenMobileEmergency}
              className="flex h-8 items-center gap-1.5 rounded-[6px] border border-neg/40 bg-neg/10 px-2.5 text-[11px] font-semibold text-neg transition-all hover:bg-neg/20 hover:border-neg/60"
              aria-label="Open emergency kill switch terminal"
            >
              <ShieldAlert size={13} className="shrink-0 anim-pulse-dot" />
...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  const toggleEnvironment = () => {
    ...
    setIsLive(next);
    try {
      localStorage.setItem("quantx_live_trading", next ? "true" : "false");
      window.dispatchEvent(new CustomEvent("quantx-env-change", { detail: { isLive: next } }));
    } catch {}
    if (next) {
      push({
        title: "Live Trading Activated",
        body: "Connected to live exchange DMA gateway. Real capital active.",
        tone: "pos",
      });
    } else {
      push({
  ...
  const onAction = (a: string) => {
    ...
    if (a === "toggle-env") { toggleEnvironment(); return; }
  ...
  return (
    <div className="min-h-screen bg-bg-primary">
      <Sidebar collapsed={collapsed} setCollapsed={setCollapsed} mobileOpen={mobileOpen} onCloseMobile={() => setMobileOpen(false)} isLive={isLive} onToggleLive={toggleEnvironment} />
      <div className={cn("transition-[padding] duration-250 ease-out", collapsed ? "lg:pl-[72px]" : "lg:pl-[248px]")}>
        <Topbar
          onMenu={() => setMobileOpen(true)}
          onOpenPalette={() => setPaletteOpen(true)}
          onOpenMobileEmergency={() => setMobileEmergencyOpen(true)}
          crumbs={crumbs}
        />
        <MarketStrip />
        <main className="relative min-h-[calc(100vh-6rem)] pb-20 md:pb-8">
          <div className="pointer-events-none absolute inset-0 grid-texture opacity-70" aria-hidden />
      ...
      <MobileEmergencyModal open={mobileEmergencyOpen} onClose={() => setMobileEmergencyOpen(false)} />
    </div>
  );
}
