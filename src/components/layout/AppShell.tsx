    ...
    <>
      ...
      >
        ...
        <div className="shrink-0 border-t border-line-subtle p-2">
          <div className={cn("mb-2 rounded-[6px] border border-warn/25 bg-warn/6", collapsed ? "p-1.5 text-center" : "px-2.5 py-2")}>
            {collapsed ? (
              <Tooltip content="Environment: PAPER TRADING" side="right">
                <span className="mx-auto block h-1.5 w-1.5 rounded-full bg-warn anim-pulse-dot" />
              </Tooltip>
            ) : (
              <>
                <div className="label-xs text-txt-disabled">Environment</div>
                <div className="mt-1 flex items-center gap-1.5">
                  <span className="h-1.5 w-1.5 rounded-full bg-warn anim-pulse-dot" />
                  <span className="label-xs text-warn">Paper Trading</span>
                </div>
              </>
            )}
          </div>

          <div className={cn("flex items-center gap-2.5 rounded-[6px] p-1.5 transition-colors hover:bg-surface-hover", collapsed && "justify-center")}>
            <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full border border-line bg-surface-high text-[10px] font-semibold text-txt-secondary">AK</span>
            {!collapsed && (
              <span className="min-w-0 flex-1">
                <span className="block truncate text-[11.5px] text-txt-primary">A. Kulkarni</span>
                <span className="block truncate text-[9.5px] text-txt-muted">Portfolio Manager · Desk 04</span>
              </span>
            )}
            {!collapsed && <IconButton label="Settings" icon={Settings} size={13} className="h-6 w-6" />}
          </div>
    ...
    <header className="sticky top-0 z-50 border-b border-line-subtle bg-bg-primary/92 backdrop-blur-xl">
      <div className="flex h-14 items-center gap-3 px-3 sm:px-4">
        ...
        <div className="ml-auto flex shrink-0 items-center gap-1">
          ...
          <span className="ml-1 flex h-8 w-8 items-center justify-center rounded-full border border-line bg-surface-high text-[10px] font-semibold text-txt-secondary" aria-label="User A. Kulkarni">AK</span>
        </div>
      </div>
    </header>
  );
